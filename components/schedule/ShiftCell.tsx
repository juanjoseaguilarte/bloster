  ...
  return (
    ...
    >
      ...
      {open && (
        ...
        >
          <div
            className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-2xl p-5 pb-8"
            onClick={e => e.stopPropagation()}
          >
            <div className="w-10 h-1 bg-gray-300 rounded-full mx-auto mb-4" />
            <h3 className="font-semibold mb-3 text-gray-800 text-center">Editar turno</h3>
            ...
            {type === 'TIME' && (
              <input
                ...
                className="w-full border border-gray-300 rounded-xl px-3 py-2.5 mb-3 text-sm text-center"
              />
            )}
            <div className="flex gap-2">
              <button onClick={() => setOpen(false)} className="flex-1 py-2.5 rounded-xl border border-gray-300 text-sm text-gray-600">Cancelar</button>
              <button onClick={handleSave} className="flex-1 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold">Guardar</button>
            </div>
          </div>
        </div>
      )}
