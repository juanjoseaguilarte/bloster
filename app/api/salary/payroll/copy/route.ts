...
export async function POST(req: NextRequest) {
  ...
  await prisma.monthlyPayroll.createMany({
    data: toCreate.map(p => ({
      ...
      advances: 0,         // Los adelantos no se copian (son del mes anterior)
      garnishments: p.garnishments,
      netAmount: p.baseAmount - p.garnishments,
      transferAmount: p.transferAmount,
      cashAmount: p.baseAmount - p.garnishments - p.transferAmount,
      notes: p.notes,
      status: 'DRAFT',
    })),
  })
